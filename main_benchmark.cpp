...
int main() {
    ...
    // PRE-CALCOLO grayscale UNA SOLA VOLTA
    ...
    for (int iter = 0; iter < NUM_ITERATIONS; ++iter) {
        ...
        for (size_t i = 0; i < preloaded_images.size(); ++i) {
            std::vector<float> grayscale = precomputed_grayscale[i];
            cpuManager.getProcessor().applySharpenFilter(grayscale, preloaded_images[i].getWidth(), preloaded_images[i].getHeight());
        }
    ...
    // --- ELIMINATO SALVATAGGIO IMMAGINI CPU ---
    ...
    CUDA_CHECK(cudaMalloc(&d_mega_input, batch_total_pixels * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&d_mega_output, batch_total_pixels * sizeof(float)));
    CUDA_CHECK(cudaMalloc(&d_widths, preloaded_images.size() * sizeof(size_t)));
    CUDA_CHECK(cudaMalloc(&d_heights, preloaded_images.size() * sizeof(size_t)));
    CUDA_CHECK(cudaMalloc(&d_offsets, preloaded_images.size() * sizeof(size_t)));
    CUDA_CHECK(cudaMemcpy(d_widths, precomputed_widths.data(), preloaded_images.size() * sizeof(size_t), cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(d_heights, precomputed_heights.data(), preloaded_images.size() * sizeof(size_t), cudaMemcpyHostToDevice));
    CUDA_CHECK(cudaMemcpy(d_offsets, precomputed_offsets.data(), preloaded_images.size() * sizeof(size_t), cudaMemcpyHostToDevice));
    std::cout << "Memoria GPU pre-allocata: " << batch_total_pixels << " pixels, " << preloaded_images.size() << " immagini" << std::endl;
    // Configurazione kernel dinamica
    cudaDeviceProp prop;
    CUDA_CHECK(cudaGetDeviceProperties(&prop, 0));
    int threads_per_block = 256;
    int num_blocks = (batch_total_pixels + threads_per_block - 1) / threads_per_block;
    for (int iter = 0; iter < NUM_ITERATIONS; ++iter) {
        std::cout << "CUDA Iterazione " << (iter + 1) << "/" << NUM_ITERATIONS << "..." << std::flush;
        auto start = std::chrono::high_resolution_clock::now();
        CUDA_CHECK(cudaMemcpy(d_mega_input, mega_buffer.data(), mega_buffer.size() * sizeof(float), cudaMemcpyHostToDevice));
        cudaManager.getProcessor().applySharpenFilterMegaBatchPreallocated(
            d_mega_input, d_mega_output, d_widths, d_heights, d_offsets,
            preloaded_images.size(), num_blocks, threads_per_block);
        auto end = std::chrono::high_resolution_clock::now();
        double time_ms = std::chrono::duration<double, std::milli>(end - start).count();
    ...
    // --- SALVATAGGIO IMMAGINI GPU DOPO IL BENCHMARK ---
    std::cout << "\nSalvataggio immagini filtrate GPU in /media/francesco/DATA/dev/Clion/Parallel_CUDA_Orlandi_Francesco/immagini_output_gpu ..." << std::endl;
    std::vector<float> gpu_output(batch_total_pixels);
    CUDA_CHECK(cudaMemcpy(gpu_output.data(), d_mega_output, batch_total_pixels * sizeof(float), cudaMemcpyDeviceToHost));
    for (size_t i = 0; i < preloaded_images.size(); ++i) {
        std::vector<float> filtered(gpu_output.begin() + precomputed_offsets[i], gpu_output.begin() + precomputed_offsets[i] + preloaded_images[i].getWidth() * preloaded_images[i].getHeight());
        normalizeBuffer(filtered);
        std::string out_path_filt = "/media/francesco/DATA/dev/Clion/Parallel_CUDA_Orlandi_Francesco/immagini_output_gpu/output_gpu_" + std::to_string(i) + ".png";
    ...
    CUDA_CHECK(cudaFree(d_mega_input));
    CUDA_CHECK(cudaFree(d_mega_output));
    CUDA_CHECK(cudaFree(d_widths));
